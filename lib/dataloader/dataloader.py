...
class VideoDataset(Dataset):
    def __init__(self, video_dataset, transform=None, time_interval=1):
        ...
        self.time_step = config.video_move_step
        ...
        for cls in cls_list:
            ...
            # online
            for begin in range(1, len(li) - (self.time_clips - 1) * time_interval):
                batch_clips = []
                if begin-self.time_step >= 0:
                    batch_clips.append(li[begin-self.time_step])
                else:
                    batch_clips.append(li[0])
                for t in range(self.time_clips):
                    batch_clips.append(li[begin + time_interval * t])
